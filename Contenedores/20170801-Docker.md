...
```bash
$ docker run --name ejercicio1 -p 80:2368 -d ghost
```

El comando anterior toma la ultima versión (:latest) de la imagen ```ghost``` y genera un contenedor a partir de ella.
Como la imagen no está disponible en nuesta máquina local, la misma se descarga desde DockerHub. Esta imagen contiene un servidor web con una aplicación para la publicación de contenido (BLOG), para verla podemos usar un navegador y acceder a http://servidorx.labs.conatel.cloud
Por el momento no nos preocupemos por las opciones `-d` y `-p 80:2368`, veremos cada una de ellas en detalle mas adelante.
...
7ed9736d1ec5        ghost               "docker-entrypoint.s…"   3 minutes ago       Up 3 minutes        0.0.0.0:80->2368/tcp   ejercicio1
```
