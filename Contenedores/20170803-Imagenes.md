...
4. A partir de la nueva imagen, crear un contenedor y verficiar que al tipear `cd ~` quedamos posicionados dentro del directorio `mi_casa`.
...
ADD config.txt /settings/config.txt
CMD bash**Construyo la nueva imagen**
```
