...
Desde el servidor de control podemos acceder a la consola del router principal por ssh (la X corresponde al número de POD asignado):
...
:warning: si al intentar conectarnos al router nos tira para afuera sin nungún mensaje, puede deberse a un problema en el que IOS busca la ssh-key en la lista de equipos conocidos, la cuál puede ver con: `ssh-add -l`. Intente agregar la clave privada a la lista de host conocidos mediante `ssh-add devops101-lab.pem` y luego verifique nuevamente con `ssh-add -l`que la misma aparece, antes de volverse a conectar: 
```
$ ssh-add -l
2048 SHA256:XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXGvhM devops101-labs.pem (RSA)
```
Si el problema persiste, puede conectarse primero al equipo `master` y desde ahí realizar el ssh al `hub` mediante la IP `10.X.254.254` o con el nombre de dominio.
